...
class ShopifyController extends Controller
{
   ...
   /* public function appMain()
    ...
    public function orderCreateWebhook(Request $request)
    {
        ...
        //DB::table('check_cron')->insert(array('test'=>1));
        $urlo = 'https://' . $shop . '/admin/customers/'.$finalOrder->customer->id.'/orders.json?limit=5';
        $urlCustomer = 'https://' . $shop . '/admin/customers/'.$finalOrder->customer->id.'.json';
        ...
        foreach($getOrders->orders as $key=>$ord)
        {
           ...
           // DB::table('check_cron')->insert(array('test'=>$ord->browser_ip));
            $product_id=$ord->line_items[0]->product_id;
            $urlp = 'https://' . $value->store_name . '/admin/products/'.$product_id.'.json';
            $sessionp = curl_init();
            curl_setopt($sessionp, CURLOPT_URL, $urlp);
    ...
    public function MisterMailScript(Request $request)
    {
        ...
        if($request->cart == 1)
        {
            if(!empty($request->ip_addr))
            {
                ...
                $subs_fields=DB::table('subscriber_fields')->where('value',$request->ip_addr)->first();
                if(!empty($subs_fields))
                {
                    $subs_data=DB::table('subscribers')->where('id',$subs_fields->subscriber_id)->first();
                    $email=$subs_data->email;
        ...
        elseif($request->pageView == 1)
        {
            ...
            else
            {
               $access_token_data=DB::table('access_tokens')->where('store_name',$request->shop)->first();
                ...
                $MailListData=DB::table('mail_lists')->where('uid',$list_uid)->first();
                $subscriberData=DB::table('subscribers')->where('mail_list_id',$MailListData->id)->where('email',$email)->first();
                $new_total_page_views=$subscriberData->total_page_views+1;
                DB::table('subscribers')->where('id',$subscriberData->id)->update(array('total_page_views'=>$new_total_page_views));

                $mister_list_array=array(
                             'EMAIL'=>$email,
                             'LAST_PAGE_VIEW'=>$request->fullUrl,
                             'LAST_SEEN' => date('Y-m-d'),
                             'TOTAL_PAGE_VIEWS' => $new_total_page_views
                            );
                $url_listt='https://app.themistermail.com/api/v1/lists/'.$list_uid.'/subscribers/store?api_token='.$value->api_token;
                $chh = curl_init();
                curl_setopt($chh, CURLOPT_URL,$url_listt);
        ...
        else
        {
            if(!empty($request->ip_addr))
            {
                $checkProduct=DB::table('products_view_ip')->where('store_name',$request->shop)->where('ip_addr',$request->ip_addr)->where('product_id',$request->product_id)->where('date',date('Y-m-d'))->first();
                if(empty($checkProduct))
                {
                    $checkProductMain=DB::table('products_view_main_ip')->where('store_name',$request->shop)->where('ip_addr',$request->ip_addr)->first();
                    if(empty($checkProductMain)){
                        DB::table('products_view_main_ip')->insert(array('store_name'=>$request->shop,'ip_addr'=>$request->ip_addr));
                    }
                    else
                    {
                       DB::table('products_view_main_ip')->where('ip_addr',$request->ip_addr)->where('store_name',$request->shop)->update(array('status'=>0));
                    }
                    DB::table('products_view_ip')->insert(array('store_name'=>$request->shop,'ip_addr'=>$request->ip_addr,'product_id'=>$request->product_id,'date'=>date('Y-m-d')));
                    echo 'done';
                }
            }
            else
            {
                ...
                if(empty($checkProduct))
                {
                    ...
                    DB::table('products_view')->insert(array('store_name'=>$request->shop,'customer_id'=>$request->customer_id,'product_id'=>$request->product_id,'date'=>date('Y-m-d')));
                    echo 'done';
                }
            }
        }   
    ...
    {
        /*$get_list_url='https://app.themistermail.com/api/v1/lists?api_token=yIlxQi9xQDdGZ1zoqwKQB5Dt4V5tPHpSQ8xJKOZQGAAuAUbaBhNo8ERmvA2K'; 
        $sessione = curl_init();
        curl_setopt($sessione, CURLOPT_URL, $get_list_url);
        curl_setopt($sessione, CURLOPT_HEADER, false);
        curl_setopt($sessione, CURLOPT_RETURNTRANSFER, true);
        ...
        while (($data = fgetcsv($file)) !== FALSE) {
            if($i > 0)
            {
                ...
                $url_listt='https://app.themistermail.com/api/v1/lists/'.$list_uid.'/subscribers/store?api_token=yIlxQi9xQDdGZ1zoqwKQB5Dt4V5tPHpSQ8xJKOZQGAAuAUbaBhNo8ERmvA2K';
                $chh = curl_init();
                curl_setopt($chh, CURLOPT_URL,$url_listt);
                curl_setopt($chh, CURLOPT_POST, 1);
                curl_setopt($chh, CURLOPT_RETURNTRANSFER, true);
